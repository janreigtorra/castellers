...
Tables:
- colles(id INTEGER PRIMARY KEY, name TEXT)
- events(id INTEGER PRIMARY KEY, date TEXT, city TEXT, place TEXT)
- event_colles(id INTEGER PRIMARY KEY, event_fk INTEGER, colla_fk INTEGER)
- castells(id INTEGER PRIMARY KEY, event_colla_fk INTEGER, castell_name TEXT, status TEXT, raw_text TEXT)
- puntuacions(castell_code TEXT, punts_descarregat INTEGER, punts_carregat INTEGER, castell_code_name TEXT)
- concurs(id INTEGER PRIMARY KEY, edition TEXT, title TEXT, date TEXT, location TEXT, colla_guanyadora TEXT, num_colles INTEGER, castells_intentats INTEGER, maxim_castell TEXT, espectadors TEXT, plaça TEXT, infobox_json TEXT, paragraphs_json TEXT)
- concurs_rankings(id INTEGER PRIMARY KEY, concurs_fk INTEGER, colla_fk INTEGER, position INTEGER, colla_name TEXT, total_points INTEGER, any INTEGER, jornada TEXT, ronda_1_json TEXT, ronda_2_json TEXT, ronda_3_json TEXT, ronda_4_json TEXT, ronda_5_json TEXT, ronda_6_json TEXT, ronda_7_json TEXT, ronda_8_json TEXT, rondes_json TEXT)
"""
...
class LLMSQLGenerator:
    ...
    def _create_query_templates(self) -> Dict[QuestionType, QueryTemplate]:
        ...
        templates[QuestionType.BEST_DIADA] = QueryTemplate(
            ...
            SELECT 
                ...
                GROUP_CONCAT(
                    CASE 
                        WHEN p.castell_code != 'Pd4' THEN p.castell_code || ' (' || c.status || ')'
                        ELSE NULL
                    END, ', ' ORDER BY 
                    CASE 
                ...
                SUM(CASE 
                    WHEN c.status = 'Descarregat' AND p.castell_code != 'Pd4' THEN COALESCE(p.punts_descarregat, 0)
                    WHEN c.status = 'Carregat' AND p.castell_code != 'Pd4' THEN COALESCE(p.punts_carregat, 0)
                    ELSE 0 
                END) AS total_punts
            FROM events e
            ...
            LEFT JOIN puntuacions p ON c.castell_name = p.castell_code_name
            WHERE 1=1
            {colla_filter}
            {year_filter}
            {location_filter}
        ...
        templates[QuestionType.BEST_CASTELL] = QueryTemplate(
            ...
            {status_filter}
            ORDER BY 
                CASE WHEN c.status = 'Descarregat' THEN p.punts_descarregat ELSE p.punts_carregat END DESC
            LIMIT :limit
            """,
            required_params=[],
        ...
        templates[QuestionType.CASTELL_STATISTICS] = QueryTemplate(
            ...
            SELECT 
                ...
                SUBSTR(GROUP_CONCAT(DISTINCT CASE WHEN c.status = 'Descarregat' THEN co.name END), 1, 400) AS primeres_colles_descarregat,
                SUBSTR(GROUP_CONCAT(DISTINCT CASE WHEN c.status = 'Carregat' THEN co.name END), 1, 400) AS primeres_colles_carregat,
                SUBSTR(GROUP_CONCAT(DISTINCT CASE WHEN c.status = 'Intent desmuntat' OR c.status = 'Intent' THEN co.name END), 1, 400) AS primeres_colles_intentat,
                COALESCE(p.punts_descarregat, 0) AS punts_descarregat,
                COALESCE(p.punts_carregat, 0) AS punts_carregat
            FROM castells c
            JOIN event_colles ec ON c.event_colla_fk = ec.id
            ...
            if len(extracted_entities["castells"]) == 1:
                ...
                if isinstance(castell, Castell):
                    ...
                    if castell.status:
                        ...
                        castell_having_filter = f"AND GROUP_CONCAT(p.castell_code || ' (' || c.status || ')') LIKE '%{castell.castell_code} ({castell.status})%'"
                        params["castell"] = castell.castell_code
                        params["status"] = castell.status
                    else:
                        # Only match castell code (no status specified)
                        castell_having_filter = f"AND GROUP_CONCAT(p.castell_code) LIKE '%{castell.castell_code}%'"
                        params["castell"] = castell.castell_code
                else:
                    # Fallback for string format
                    castell_having_filter = f"AND GROUP_CONCAT(p.castell_code) LIKE '%{castell}%'"
                    params["castell"] = castell
            elif len(extracted_entities["castells"]) > 1:
                ...
                for castell in extracted_entities["castells"]:
                    if isinstance(castell, Castell):
                        if castell.status:
                            # Match both castell code and status together
                            castell_conditions.append(f"GROUP_CONCAT(p.castell_code || ' (' || c.status || ')') LIKE '%{castell.castell_code} ({castell.status})%'")
                        else:
                            # Only match castell code (no status specified)
                            castell_conditions.append(f"GROUP_CONCAT(p.castell_code) LIKE '%{castell.castell_code}%'")
                    else:
                        # Fallback for string format
                        castell_conditions.append(f"GROUP_CONCAT(p.castell_code) LIKE '%{castell}%'")
        ...
        if extracted_entities.get("anys"):
            if len(extracted_entities["anys"]) == 1:
                ...
                if question_type in [QuestionType.CONCURS_RANKING, QuestionType.CONCURS_HISTORY]:
                    ...
                    year_filter = f"AND strftime('%Y', e.date) = '{year}'"
                params["year"] = year
                ...
                if question_type in [QuestionType.CONCURS_RANKING, QuestionType.CONCURS_HISTORY]:
                    ...
                    year_filter = f"AND strftime('%Y', e.date) IN ({anys})"
    ...
    def _create_custom_sql_query(self, question: str, entities: Dict, llm_call_func) -> Tuple[str, Dict[str, any]]:
        ...
        sql_prompt = f"""
            ...
            Tens una base de dades SQLite amb l'estructura següent:
            ...
            Genera una única consulta SQL **completa i vàlida per SQLite** que respongui aquesta pregunta.
            - Usa noms exactes de taules i columnes segons l'esquema.
            - Si cal filtrar per any, pots utilitzar `strftime('%Y', e.date) = :year`.
            ...
            - Utilitza paràmetres amb `:nom` en lloc d'inserir valors directament.
...
**Instruccions específiques per a millor diada:**
...
2. Proporciona **TOT EL CONTEXT** de la millor actuació:
   ...
   - Els castells fets (excloent Pde4/Pd4/Pde5/Pd5)
   - Estat de cada castell (Descarregat/Carregat/Intent desmuntat)
