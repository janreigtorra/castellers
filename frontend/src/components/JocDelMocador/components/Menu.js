...
const Menu = ({ onStartGame, isLoading, onBack, theme, gameSettings, onSettingsChange, onColorChange, selectedColor }) => {
  ...
  const getDescription = () => {
    ...
    if (hasColles) {
      if (selectedColles.length === 1) {
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents a la colla {selectedColles[0]}.</span></>;
        ...
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents a les colles {otherColles} i {lastColla}.</span></>;
      } else {
        const firstSix = selectedColles.slice(0, 6).join(', ');
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents a les colles {firstSix}... (i {selectedColles.length - 6} més).</span></>;
      }
    }
    ...
    if (hasYears) {
      ...
      if (sortedYears.length === 1) {
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents a l'any {sortedYears[0]}.</span></>;
      } else if (sortedYears.length <= 6) {
        const lastYear = sortedYears[sortedYears.length - 1];
        const otherYears = sortedYears.slice(0, -1).join(', ');
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents als anys {otherYears} i {lastYear}.</span></>;
      } else {
        const firstSix = sortedYears.slice(0, 6).join(', ');
        return <>{baseText}<br /><span className="joc-mocador-filter-text">Preguntes referents als anys {firstSix}... (i {sortedYears.length - 6} més).</span></>;
      }
    }
  ...
  return (
    <div className="joc-mocador-menu">
      {/* Left sidebar with options */}
      <div className="joc-mocador-menu-sidebar">
        <h3 className="joc-mocador-sidebar-title">Personalitza la teva partida</h3>
        
        {/* Number of questions */}
        <div className="joc-mocador-sidebar-option">
          <label className="joc-mocador-option-label">Nombre de preguntes</label>
          <div className="joc-mocador-slider-option">
            <input
              type="range"
              min="5"
              max="20"
              ...
              className="joc-mocador-option-slider"
              disabled={isLoading}
            />
            <span className="joc-mocador-option-value">{numQuestions}</span>
          </div>
        </div>

        {/* Colla selection */}
        <div className={`joc-mocador-sidebar-option ${selectedYears.length > 0 ? 'joc-mocador-option-disabled' : ''}`}>
          <label className="joc-mocador-option-label">
            Colles
            {selectedYears.length > 0 && <span className="joc-mocador-option-hint"> (neteja anys primer)</span>}
          </label>
          <MultiSelect
        ...
        {/* Year selection */}
        <div className={`joc-mocador-sidebar-option ${selectedColles.length > 0 ? 'joc-mocador-option-disabled' : ''}`}>
          <label className="joc-mocador-option-label">
            Anys
            {selectedColles.length > 0 && <span className="joc-mocador-option-hint"> (neteja colles primer)</span>}
          </label>
          <MultiSelect
      ...
      {/* Main content */}
      <div className="joc-mocador-menu-content">
        <div className="joc-mocador-icon-container">
          <img 
            src={isLoading ? "/xiquet_images/xiquet_loading.png" : "/xiquet_images/xiquet_go.png"}
            alt="Xiquet" 
            className="joc-mocador-xiquet-icon"
          />
        </div>
        <div className="joc-mocador-menu-text">
          <h1 className="joc-mocador-title">El Joc del Mocador</h1>
          <p className="joc-mocador-subtitle">El joc on pots posar a prova tot el que saps sobre el món casteller. En sabràs més que jo?</p>
          <p className="joc-mocador-description">{getDescription()}</p>
          <button 
            className={`joc-mocador-play-btn ${buttonPulse ? 'pulse' : ''}`}
            onClick={onStartGame}
            disabled={isLoading}
          >
