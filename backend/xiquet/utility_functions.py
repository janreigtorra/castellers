...
def get_anys_subset(question: str, top_n: int = 5) -> str:
    ...
    # But exclude if it's part of a castell reference like "de 7", "de 8", etc.
    ...
    for match in two_digit_matches:
        ...
        # Skip if it's a single digit castell height (6, 7, 8, 9) - these are castells, not years
        if year_2digit in [2,3,4,5,6, 7, 8, 9]:
            continue
