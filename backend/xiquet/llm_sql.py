...
class QuestionType(Enum):
    ...
    CASTELLS_LIST = "castells_list"     # Llista de castells fets (filtrable per colla, diada, any, lloc, status, gamma)
    ...
    COLLES = "colles"  # Quines colles han fet X (filtrable per castell, diada, lloc, any)
...
class QueryTemplate:
    ...
    llm_context_limit: int = LLM_CONTEXT_LIMIT  # Limit of rows passed to LLM for summarization
...
class LLMSQLGenerator:
    ...
    def _create_query_templates(self) -> Dict[QuestionType, QueryTemplate]:
        ...
        templates[QuestionType.BEST_DIADA] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=5  # Best diada usually has few results, 5 is enough
        )
        ...
        templates[QuestionType.BEST_CASTELL] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=10  # Best castell usually has few results
        )
        ...
        templates[QuestionType.CASTELL_HISTORIA] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=15  # Castell historia can have many results, need more context
        )
        
        # Template for castells list (quins castells van fer)
        templates[QuestionType.CASTELLS_LIST] = QueryTemplate(
            question_type=QuestionType.CASTELLS_LIST,
            sql_template="""
            SELECT 
                {gamma_select}
                c.castell_name,
                c.status,
                STRING_AGG(DISTINCT e.name, ', ' ORDER BY e.name) AS diades,
                STRING_AGG(DISTINCT e.place, ', ' ORDER BY e.place) AS places,
                STRING_AGG(DISTINCT e.city, ', ' ORDER BY e.city) AS cities,
                STRING_AGG(DISTINCT co.name, ', ' ORDER BY co.name) AS colles,
                MIN(e.date) AS first_date,
                MAX(CASE 
                    WHEN c.status = 'Descarregat' THEN COALESCE(p.punts_descarregat, 0)
                    WHEN c.status = 'Carregat' THEN COALESCE(p.punts_carregat, 0)
            ...
            {status_filter}
            {gamma_filter}
            GROUP BY c.castell_name, c.status{gamma_group_by}
            ORDER BY 
                MAX(CASE 
                    WHEN c.status = 'Descarregat' THEN COALESCE(p.punts_descarregat, 0)
                    WHEN c.status = 'Carregat' THEN COALESCE(p.punts_carregat, 0)
                    ELSE 0 
                END) DESC,
                MIN(TO_DATE(e.date, 'DD/MM/YYYY')) ASC,
                c.castell_name ASC
            LIMIT %(limit)s
            """,
            required_params=[],
            optional_params=["colla", "year", "location", "diada", "status", "gamma"],
            description="List of castells (can be filtered by colla, year, location, diada, status, gamma)",
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=20  # Castells list can have many results, need more context
        )
        ...
        templates[QuestionType.LOCATION_ACTUATIONS] = QueryTemplate(
            ...
            GROUP BY
                ...
                city,
                colla_name
            ORDER BY total_punts DESC, TO_DATE(date, 'DD/MM/YYYY') DESC
            LIMIT %(limit)s
            """,
            required_params=[],
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=8  # Location actuations can have multiple results
        )
        ...
        templates[QuestionType.YEAR_SUMMARY] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=15  # Year summary can have many colles, need more context
        )
        ...
        templates[QuestionType.FIRST_CASTELL] = QueryTemplate(
            ...
            sql_template="""
            WITH first_castells AS (
                SELECT DISTINCT ON (c.castell_name, c.status)
                    EXTRACT(YEAR FROM TO_DATE(e.date, 'DD/MM/YYYY')) AS year,
                    e.name AS event_name,
                    e.date,
                    e.place,
                ...
                {status_filter}
                {gamma_filter}
                ORDER BY c.castell_name, c.status, TO_DATE(e.date, 'DD/MM/YYYY') ASC
            )
            SELECT *
            FROM first_castells
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=15  # First castell returns max 4 rows (one per status)
        )
        ...
        templates[QuestionType.CASTELL_STATISTICS] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=5  # Statistics queries return aggregated data, usually 1 row
        )
        ...
        templates[QuestionType.CONCURS_RANKING] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=24  # Concurs rankings can have many participants
        )
        ...
        templates[QuestionType.CONCURS_HISTORY] = QueryTemplate(
            ...
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=24  # Concurs history can span multiple years
        )
        
        # Template for colles (quines colles han fet X)
        templates[QuestionType.COLLES] = QueryTemplate(
            question_type=QuestionType.COLLES,
            sql_template="""
            {query_variant}
            """,
            required_params=[],
            optional_params=["castell", "diada", "location", "year", "gamma", "status"],
            description="Quines colles han fet X (filtrable per castell, diada, lloc, any, gamma, status). Si hi ha diada o lloc: mostra colla, diada, lloc, any, castells fets. Si hi ha castell/gamma: mostra colla, castell, estadístiques per status",
            default_limit=SQL_RESULT_LIMIT,
            llm_context_limit=20  # Colles queries can have many results
        )
    ...
    def generate_sql_query_from_template(self, question: str, extracted_entities: Dict, question_type: QuestionType) -> Tuple[str, Dict[str, any]]:
        ...
        for param in template.required_params:
            if param == "colla" and not extracted_entities.get("colla"):
                ...
                # For castell, accept either castells list OR gamma (gamma is an alternative way to filter by castells)
                if not extracted_entities.get("castells") and not extracted_entities.get("gamma"):
        ...
        gamma_group_by = ""  # For GROUP BY clause when gamma is used
        if extracted_entities.get("gamma"):
            ...
            if "specific" in gamma_def:
                ...
                # Create variations: for pilars, add both "pdX" and "pdeX" formats
                # because in puntuacions table:
                # - castell_code is "pd8fm" (lowercase, no "de")
                # - castell_code_external is "Pde8fm" (with "de", may have uppercase)
                specific_castells_variations = set(specific_castells_lower)
                for c in specific_castells_lower:
                    # If it's a pilar (starts with 'p'), add variation with 'de'
                    if c.startswith('p') and len(c) > 1 and c[1] == 'd' and 'de' not in c:
                        # Replace first 'd' with 'de' (e.g., "pd8fm" -> "pde8fm")
                        variation = 'p' + 'de' + c[2:]  # "pd8fm" -> "pde8fm"
                        specific_castells_variations.add(variation)
                    # Also add variation without 'de' if it has 'de' (e.g., "pde8fm" -> "pd8fm")
                    elif c.startswith('p') and 'de' in c:
                        variation = c.replace('de', 'd', 1)  # "pde8fm" -> "pd8fm"
                        specific_castells_variations.add(variation)
                
                # Check castell_name directly first (most reliable), then puntuacions table
                # This handles cases where castells might not be in puntuacions table
                # Also handles pilars which can be stored in either format
                gamma_filter = "AND (LOWER(c.castell_name) IN %(gamma_param)s OR LOWER(p.castell_code) IN %(gamma_param)s OR LOWER(p.castell_code_external) IN %(gamma_param)s)"
                ...
                # Add gamma to GROUP BY if used in SELECT
                gamma_group_by = ", gamma_filtrada"
        ...
        # Special handling for COLLES query type - generate dynamic query based on filters
        query_variant = ""
        if question_type == QuestionType.COLLES:
            has_diada_or_location = bool(extracted_entities.get("diades") or extracted_entities.get("llocs"))
            has_castell_or_gamma = bool(extracted_entities.get("castells") or extracted_entities.get("gamma"))
            
            if has_diada_or_location:
                # Variant 1: Group by colla, show colla, diada, lloc, any, castells fets
                query_variant = """
                SELECT 
                    co.name AS colla_name,
                    e.name AS diada,
                    e.city AS lloc,
                    EXTRACT(YEAR FROM TO_DATE(e.date, 'DD/MM/YYYY')) AS any,
                    STRING_AGG(
                ...
                {status_filter}
                {gamma_filter}
                GROUP BY co.name, e.name, e.city, e.date, EXTRACT(YEAR FROM TO_DATE(e.date, 'DD/MM/YYYY'))
                ORDER BY EXTRACT(YEAR FROM TO_DATE(e.date, 'DD/MM/YYYY')) DESC, e.date DESC
                LIMIT %(limit)s
                """
            elif has_castell_or_gamma:
                # Variant 2: Group by colla and castell, show colla, castell, estadístiques per status
                query_variant = """
                SELECT 
                    co.name AS colla_name,
                    c.castell_name,
                    ...
                    MIN(CASE WHEN c.status = 'Descarregat' THEN TO_DATE(e.date, 'DD/MM/YYYY') END) AS primera_data_descarregat,
                    MIN(CASE WHEN c.status = 'Carregat' THEN TO_DATE(e.date, 'DD/MM/YYYY') END) AS primera_data_carregat,
                    MIN(TO_DATE(e.date, 'DD/MM/YYYY')) AS primera_data
                FROM colles co
                JOIN event_colles ec ON co.id = ec.colla_fk
                JOIN events e ON ec.event_fk = e.id
                JOIN castells c ON ec.id = c.event_colla_fk
                ...
                {status_filter}
                {gamma_filter}
                GROUP BY co.name, c.castell_name
                ORDER BY MAX(TO_DATE(e.date, 'DD/MM/YYYY')) DESC, co.name, c.castell_name
                LIMIT %(limit)s
                """
            else:
                # Default: If no specific filter, show variant 1 (by diada/location) but without those filters
                query_variant = """
                SELECT 
                    co.name AS colla_name,
                    e.name AS diada,
            ...
            # For other query types, use the template as-is
            query_variant = template.sql_template
        
        # Build the final SQL query
        sql_query = query_variant.format(
    ...
    def get_llm_context_limit(self, sql_query_type: str) -> int:
        ...
        Args:
            sql_query_type: The SQL query type (e.g., "castell_historia", "millor_diada")
            
        Returns:
            The LLM context limit for this query type, or default if not found
        """
        ...
        query_type_mapping = {
            ...
            "castells_list": QuestionType.CASTELLS_LIST,
            ...
            "colles": QuestionType.COLLES
        }
        
        question_type = query_type_mapping.get(sql_query_type)
        if question_type and question_type in self.templates:
            return self.templates[question_type].llm_context_limit
    ...
    def _create_custom_sql_query(self, question: str, entities: Dict, llm_call_func) -> Tuple[str, Dict[str, any]]:
        ...
        sql_prompt = f"""
            ...
            - **CRÍTIC**: Utilitza paràmetres amb `%(nom)s` en lloc d'inserir valors directament.
            - **CRÍTIC**: Els paràmetres NO han d'estar dins de cometes simples. Escriu `WHERE co.name = %(colla)s` NO `WHERE co.name = '%(colla)s'`.
            - Per LIKE amb paràmetres, usa `LIKE %(param)s` i després passa el valor amb `%` al diccionari de paràmetres (ex: `params['param'] = '%valor%'`).
        ...
        try:
            ...
            # Fix common mistake: parameters inside quotes
            # Replace patterns like '%(param)s' with %(param)s
            sql_query = re.sub(r"'%\((\w+)\)s'", r'%(\1)s', sql_query)
            # Also fix double quotes
            sql_query = re.sub(r'"%\((\w+)\)s"', r'%(\1)s', sql_query)
            # Fix LIKE patterns with parameters: LIKE '%(param)s' -> LIKE %(param)s
            sql_query = re.sub(r"LIKE\s+'%\((\w+)\)s'", r'LIKE %(\1)s', sql_query, flags=re.IGNORECASE)
            sql_query = re.sub(r"LIKE\s+\"%\((\w+)\)s\"", r'LIKE %(\1)s', sql_query, flags=re.IGNORECASE)
    ...
    def execute_sql_query(self, sql_query: str, params: Dict[str, any]) -> List:
        ...
        try:
            ...
            # Log the technical error for debugging but raise user-friendly exception
